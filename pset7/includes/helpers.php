<?php
    ...
    /**
     * Logs transactions into history.
     */
    function history($type, $symbol, $shares, $price)
    {
        // checks current time
        $datetime = date('d/m/Y H:i');
        
        // logs transaction history db
        $logcheck = cs50::query("INSERT INTO history (user_id, type, date_time, symbol, shares, price) VALUES (?, ?, ?, ?, ?, ?)", $_SESSION["id"], $type, $datetime, $symbol, $shares, $price);
        
        // if insertion fails, display error
        if($logcheck === false)
        {
            return false;
        }
