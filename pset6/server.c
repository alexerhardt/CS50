...
char* indexes(const char* path)
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        // If index.php is found, add it to path and return the value
        if( strcmp(namelist[i]->d_name, "index.php") == 0 )
        {
            char* index_path = NULL;
            if( ( index_path = malloc( strlen(path) + strlen(namelist[i]->d_name) + 1 ) ) == NULL )
            {
                error(500);
                return NULL;
            }
            strcpy(index_path, path);
            strcat(index_path, namelist[i]->d_name);
            freedir(namelist, n);
            return index_path;
        }
        
        // If index.html is found, same as above
 ...
 * Inspired by CS50's GetString(): https://mirror.cs50.net/library50/c/cs50-library-c-3.0/cs50.c
 */
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // Minimize the buffer and copy file contents into it
    char* minimal = calloc( n * sizeof(char), 1 );
    if (minimal == NULL)
    {
      free(buffer);
      return false;
    ...
    // Point content and length to values and wrap it up
    *content = minimal;
    *length = n;
    return true;
}
...
const char* lookup(const char* path)
{
    ...
    // Pinpoint where the file extension begins
    char* haystack = strdup(path);
    ...
    // Initialize array of pointers to file extensions and their MIME types
    struct filespecs* file_tab[] =
    {
        &(struct filespecs) {".html", "text/html"},
        &(struct filespecs) {".css", "text/css"},
        &(struct filespecs) {".gif", "image/gif"},
        &(struct filespecs) {".ico", "image/x-icon"},
        &(struct filespecs) {".jpg", "image/jpeg"},
        &(struct filespecs) {".js", "text/javascript"},
        &(struct filespecs) {".php", "text/x-php"},
        &(struct filespecs) {".png", "image/png"},
    };
    
    // Iterate over file extensions, if file extension matches, return MIME type
    for(int i = 0; i < 8; i++)
    {
        if( strcasecmp( file_tab[i]->file_ext, needle) == 0 )
        {
        return file_tab[i]->mime_type;
        }
    }
    
    // Return NULL if file type is not found
    return NULL;
}
...
{
    // Initializes abs_path and query
    for(int i = 0; i < LimitRequestLine; i++)
    {
        abs_path[i] = 0;
        query[i] = 0;
    }
    ...
    // Pinpoint positions of request-target, HTTP version and CRLF, and check for quotation marks
    char* haystack = strdup(line);
    char* rt_needle = strchr(haystack, ' ');
    char* http_needle = strstr(haystack, " HTTP/");
    char* crlf_needle = strstr(haystack, "\r\n");
    char* qmrk_needle = strchr(haystack, '\"');
    
    // Return 400 error if the line is not correct
    if ( spaces != 2 || http_needle == NULL || crlf_needle == NULL || qmrk_needle != NULL )
    {
        error(400);
        return false;
    }
    
    // Find method, check if it's GET, return error otherwise
    char method[ rt_needle - haystack + 1 ];
    for(int i = 0; i < (rt_needle - haystack + 1); i++)
    {
        method[i] = 0;
    }
    strncpy(method, haystack, rt_needle - haystack);
    ...
    // Pinpoint position of request-target, return error if not starting with slash character
    if( *( rt_needle + 1 ) != '/' )
    {
        error(501);
        return false;
    ...
    // Find if query exists, and build abs-path and query accordingly
    haystack = rt_needle + 1;
    char* q_needle = strstr(haystack, "?");

    if(q_needle != NULL)
    {
        strncpy(abs_path, haystack, q_needle - haystack);
        haystack = q_needle + 1;
        strncpy(query, haystack, http_needle - haystack);
    }
    
    else
    {
        strncpy(abs_path, haystack, http_needle - haystack);
        query = "\0";
    }
    
    // Check HTTP version
    haystack = http_needle + 1;
    if( ( strncmp(haystack, "HTTP/1.1", 8) ) != 0 )
    {
        error(505);
        return false;
    }
